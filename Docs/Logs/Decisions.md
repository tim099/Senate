...
## D26 · 常駐 Server：自動啟動 ＋ 獨立出貨 exe（2026-09-14／09-15，Tim 拍板，TASK-0209 A 段）

> ⛔ **不改 D20** —— 這是新增一條。D20 的三格在它被拍的那天全部成立，
> 而它們沒有壞掉，只是其中一格的前提換了（銀行搬進來 ⇒ 沒有人在看的自動流程要動錢）。

**修正 D20 的第三格（誰啟動）**：由「人手動、撞到沒 Server 就 exit 3」改為
**CLI 偵測不到就自動拉起一顆**。

| D20 的理由 | 現在為什麼不成立／怎麼處理 |
|---|---|
| 「兩顆 CLI 同時發現沒 Server 的 race」 | ⭐ 判準換成「**確保有一顆**」不是「起我的那顆」：N 顆一起 spawn、**單例 OS 鎖**只讓一顆活，輸掉的那幾顆 CLI 等的是 `Probe().IsRunning`，**不看 pid** ⇒ 它們不該報錯（活體：4 顆同時委派，4 顆全 Success、鎖擋了 3 次、registry 1 顆） |
| 「違反 R18 不做降級路」 | 🔴 **R18 只翻了「手動」那半，「不降級」那半沒翻**：拉不起來仍然 exit 3，**絕不改成本地跑** |

**為什麼非翻不可**：銀行搬進 Server 之後，被「Server 沒開」擋住的是
**跨日保管費／領薪／發文計酬**那些沒有人在看的流程 —— 而「請先手動 start」這句話
在沒有人在看的路上等於沒有人會讀到它。

**兩個失敗態不共用出口**（⚠ 這是同一族的老病）：
`autostart_timeout`（spawn 了但沒等到 ⇒ **不知道它慢還是死了**）與
`autostart_failed`（連 spawn 都失敗 ⇒ **確定沒起來**）是**兩個不同的 `delegate_failure` 值 ＋ 兩段不同的話**。
⛔ 不重試不退避 —— 自動重試會把「環境壞了」變成一個每次慢 N 秒而且永遠不報錯的黑洞。

**啟動 log 的落檔責任歸 child**（v3，前兩版都壞在同一句話上）：
parent 接管線 ⇒ parent 跑完就退，非同步讀取器跟著死 ⇒ 輸掉單例鎖那幾顆的訊息**一筆都留不下**，
而它消失的樣子是**一個格式正確的空檔**。
📌 一般形：**接在別人生命週期上的日誌，會在那個人先走的時候一起消失。**

**出貨形狀（A7，2026-09-15 拍：(甲) 自足單檔）**：
`build.sh` 兩次 publish、同一個 `build_id` ⇒ `publish/senate.exe`（78MB，含 GUI 原生層）
＋ `publish/server/senate-server.exe`（72MB，**零 cimgui/glfw3**）。自動啟動優先起後者。

被否掉的 (乙) framework-dependent（~1MB）理由只有一句：
**它的失效模式是「那台機器沒有 .NET runtime」，而它發作的位置正好是自動啟動 ——
也就是沒有人在看的那條路。** ⇒ 用 72MB 磁碟換掉一整類環境失敗，**代價具名，不是漏算**。

🩸 **而改名當場讓一行定語開始說謊**：`ServerHost` 的 `registered_by` 比對的是寫死的
`"Senate.Server"`，出貨名改成 `senate-server` 之後比對失效，它回「senate server start」——
而真的起它的是那顆獨立 exe。⚠ 沒有任何一層報錯。
⇒ 判準倒過來寫：**只認 CLI 自己那個名字，其餘一律報實際檔名**；
未來再改名，最壞是印一個新名字（真的），不是印一個舊動作（假的）。

---

## D27 · 新銀行與舊 Treasury **並存**：遷移前新銀行是測試用，實際以舊系統為準（2026-09-15，Tim 拍板）

**拍的是什麼**：`SCP_Bank*`（Senate 原生銀行）與 Unity 側的 `UCL_TreasuryLedger`
**完全獨立、長期並存**，而且：

> 🔴 **遷移前新銀行一律視為測試用；「我有多少錢」的唯一權威仍是舊系統。**

**這一條取消了 TASK-0209 原本的 C8**（「全部消費端切到新系統，必須與 B 同一次上線」）。
C8 的理由是「只接 Server 而 Editor 仍直寫磁碟 ＝ 兩個寫入端，比現在更糟」——
⚠ **那句話的前提是兩邊在搶同一筆錢**。前提換掉之後它就不成立了：
兩套系統各自寫各自的帳本，**它們不是同一本帳**。

⇒ 消費端（Editor 側 `UCL_TreasuryLedger` 全庫 **86 處／26 檔**：Credit 29／Debit 17／GetBalance 24）
**一行都不動**。

**未來遷移的射程（拍板當天就寫死，⛔ 不是等到那天再想）**：
- 只搬**開帳金額**（`opening_balance` 一筆一戶），⛔ 不搬歷史分錄
- 只遷**有綁 persona 的帳戶 ＋ 央行**；其餘（幽靈帳戶、`discord:*`、別區綁定）一律不遷
- 舊 ledger **不刪不搬**（它還在服役）

**⚠ 並存期唯一的新風險，以及它的處置**：
「我有多少錢」會有**兩個答案**，而**兩邊都不會報錯**（它們各自都對，只是在回答不同的問題）。
⇒ 處置是**在輸出上帶定語**：新銀行的每一次查詢與收付都明說
「這是新銀行（遷移前＝測試用），實際餘額以舊系統為準」。
📌 ⛔ 不靠「大家記得」—— 記得是這個系統最不能依賴的東西。
