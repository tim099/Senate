...
| | 讀數 |
|---|---|
| ✅ **親代鏈** | `Claude.exe`（`C:\Program Files\WindowsApps\Claude_*`，MSIX 封裝）→ `claude-code` → `bash` → agent 起的任何東西 |
| ✅ **失敗有名字** | MSIX 部署 `0x80073D02`「必須先關閉 `Claude_<舊版>`」 |
| ✅ **時間軸**（09-16） | `08:12 id=658 延後註冊` → `09:10 id=419/404 失敗` → `09:48 id=400 成功`（最後那格是使用者關掉 senate 之後） |
| ✅ **不是偶發** | `id=658` 在 9/4、9/7、9/9、9/11、9/16 各出現 ⇒ **每次 Claude 更新都會走到這一格** |
| ⛔ **仍然沒量到** | 那顆 senate 是不是真的帶著**封裝 app 的套件身分**。要量它得讓它再擋一次更新，而那筆帳落在使用者的機器上 |

⇒ **常駐 Server 那條路已經治掉**：`ServerAutoStart` 改走 WMI `Win32_Process.Create`，
新行程的親代是 `WmiPrvSE`（實測兩次可重現）⇒ 不在任何人的行程樹底下。
⭐ 脫樹對「handle 繼承」與「身分繼承」**兩種機制都有效**，所以它不必等那一格量到。

⇒ 而**本節這顆 GUI 視窗刻意不跟著改走脫樹**：底下那條 `[ -t 1 ]` 已經讓 agent 一顆都不開，
而「不種就不必拔」比「種了再脫樹」少一層會失敗的步驟。

> ⚠ 撞到「Claude Code 起不來」時**先看有沒有 `senate.exe` 在跑** —— 常駐 Server **沒有視窗**，
> 所以除了工作管理員以外看不到它。`senate server stop` 一行就能收。
