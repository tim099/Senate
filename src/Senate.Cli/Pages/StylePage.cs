...
//           人在視窗裡按、agent 用 `ui --click home/open/style` 進來再 `--click style/big`、
//           文字模式看得到現在停在哪一頁（麵包屑）、截圖證明它真的畫出來了。
// 數值影響：按下尺寸會**寫回 senate.local.json**（走 SenateModel.ApplySize → SenateUiStore，
//           寫完回讀確認才回報成功）。除此之外唯讀。
using SCP.Core.Gui;

namespace Senate.Cli.Pages;

public sealed class StylePage : SCP_GuiToolPage
{
    readonly SenateModel m_Model;

    /// <summary>`: base()` 讓 [CallerFilePath] 填 SourceFilePath（隱式 base() 會是 null）。</summary>
    public StylePage(SenateModel iModel) : base() { m_Model = iModel; }
