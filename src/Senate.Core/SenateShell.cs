...
public static class SenateShell
{
    ...
    public static string Reveal(string iPath, string? iRepoRoot = null)
    {
        ...
        // 成功 ⇒ **不回話**（Tim 2026-09-01 拍板）。
        // 空字串在 SCP_GuiToolPage 那側被收斂成「沒有話要說」⇒ 畫面上不多任何一行。
        // 理由：檔案總管**自己跳出來了**，那就是讀數；再補一句「✓ 已顯示」是每按一次就多一條
        // 沒有人會讀的訊息，而訊息一多，真正該被看見的那條（開不起來）就被淹掉了。
        // ⚠ 上面 Linux 那條分支**仍然回話**，而那不是不一致：它說的是「只開到資料夾、
        //   沒有選取到檔案」—— 那是使用者拿到的東西與他預期的不同，屬於失敗的一種。
        return string.Empty;
    }
