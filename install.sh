...
#
# ⚠ 本檔是**唯一**的安裝實作（`install.ps1` 已於 2026-09-22 退場，Tim：沒再用）。
#   🩸 它退場前這裡寫的是「本檔與 install.ps1 是同一套規格的兩個宿主實作，移除清單改一邊要同時改另一邊」——
...
if [ "$1" = "--uninstall" ]; then
  ...
  # ② build 產物（可重建 ⇒ 預設就移除）
  #    ⚠ 這份清單以前要與 install.ps1 的 $aArtifacts 保持一致（那支已退場 ⇒ 現在只有這一份）。
  #
